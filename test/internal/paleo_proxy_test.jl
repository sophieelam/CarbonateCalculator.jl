...
@testset "ReferenceDataTestCase: Test boron isotopes" begin
    ...
    @testset "Bisotopes" begin
        ...
        for i in 1:n
            ...
            test_sys = whole_system(pHtot=pHtot, δBT=δBT, BT=BT, DIC=DIC, temp_c=T, sal=S)

            # Test boron_isotopes
            check_A = boron_isotopes(δBOH₄=test_sys.δBOH₄, δBT=test_sys.δBT, temp_c=T, sal=S)
            @test test_sys.pHtot ≈ check_A.pHtot rtol=1e-10
            
            # Test boron_system
            check_B = boron_system(δBOH₄=test_sys.δBOH₄, δBT=test_sys.δBT, BT=test_sys.BT, temp_c=T, sal=S)
            @test test_sys.pHtot ≈ check_B.pHtot rtol=1e-10

            # Test whole_system
            check_CB = whole_system(δBOH₄=test_sys.δBOH₄, δBT=test_sys.δBT, DIC=test_sys.DIC, BT=test_sys.BT, temp_c=T, sal=S)
